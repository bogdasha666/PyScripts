...
def start_script(path):
    try:
        ...
        # Убираем подавление вывода, чтобы видеть ошибки
        return subprocess.Popen([sys.executable, abs_path], env=env)
...
class ScriptGUI(QWidget):
    def __init__(self):
        ...
        if not gsi_proc or gsi_proc.poll() is not None:
            ...
            if not gsi_proc or gsi_proc.poll() is not None:
                print("[error] Не удалось запустить GSI сервер для CustomBinds! Проверьте ошибки выше (например, занят порт 3000 или пайп уже существует). Если GSI.py уже запущен вручную — закройте его!")
            else:
